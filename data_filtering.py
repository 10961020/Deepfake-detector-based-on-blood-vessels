...
if __name__ == '__main__':
    ...
    config = {
        'input_path': r'E:\zhangtong\bloodshot_research\face_img\STYLEGAN',
        'output_path': r'E:\zhangtong\bloodshot_research\output',  # network weight
        "save_img": r'E:\zhangtong\bloodshot_research\face_img\val',  # filtering save path
        'facedetector_path': r'E:\zhangtong\bloodshot_research/shape_predictor_68_face_landmarks.dat',
    }
    data_filter()


    # for roots, dirs, files in os.walk(r'E:\zhangtong\bloodshot_research\face_img\val_random'):
    #     for file in files:
    #         i, j = os.path.splitext(file.split('\\')[-1])
    #         os.replace(os.path.join(roots, file), os.path.join(roots, i[1:]+j))